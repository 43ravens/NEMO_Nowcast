...
@patch('nemo_nowcast.message_broker.lib.load_config')
...
class TestMain:
    ...
    def test_commandline_parser(
        self, m_run, m_arg_parser, m_load_config, m_logging,
    ):
        message_broker.main()
    ...
    def test_logging(
        self, m_logger, m_run, m_arg_parser, m_load_config, m_logging,
    ):
        m_arg_parser().parse_args.return_value = Mock(
            config_file='nowcast.yaml')
